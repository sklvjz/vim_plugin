...
"/ is common used
let g:cscope_file = g:base_dir . "/.CIndexDB" . "/cscope.out"
let g:lines_count_file = g:base_dir . "/.CIndexDB" . "/lines_count.txt"
let g:files_list = g:base_dir . "/.CIndexDB" . "/files.list"

" tags files contains some information about completion
let g:tags_file = g:base_dir . "/.CIndexDB" . "/tags"

" env.vim stores some settings for this project, such as include's dir, etc.
let g:env_file = g:base_dir . "/.CIndexDB" . "/env.vim"

if filereadable("./.CIndexDB/in.vim")
      source ./.CIndexDB/in.vim
      if g:support_cproject == 0
          finish
      endif
else
    finish
...
let s:tags_cmd = "!ctags -L " . g:files_list . " --c-kinds=+p --c++-kinds=+p --fields=+iaS --extra=+q -f " . g:tags_file
...
func! CSCOPE_FILES(project_base_dir, files_list)
	...
	if filereadable(a:files_list)
		let lines = readfile(a:files_list)
			...
			" linux
			silent! execute g:cscope_cmd . a:files_list . " -f " . g:cscope_file
...
func! Get_project_files(project_base_dir)
	if a:project_base_dir != ''
        ...
        if filereadable("./.CIndexDB/header_src.conf")
            let incfile_list = readfile("./.CIndexDB/header_src.conf")
            for dirlist in incfile_list
                let filelist = filelist . expand(dirlist . '/**/*.[ch]')
            endfor
        endif
		
        let file_list = split(filelist, "\n")

        "extend(file_list,filelist1)
		return file_list
	else
		return []
	endif
